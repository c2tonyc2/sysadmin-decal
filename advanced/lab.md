...
Network interfaces represent a point of connection between a computer and a network.  Typically network interfaces are associated with a physical piece of hardware i.e. a network interface card.  However interfaces can be entirely implemented in software and have no physical counterpart -- take the loopback interface `lo` for example.  `lo` is a virtual interface, it simulates a network interface with only software.
...
* `ip_default_ttl`
This is a simple file that configures the default TTL for outbound IP packets.
* `ip_local_port_range`
...
* `tcp_retries2`
This file limits the number of re-transmissions before killing active connection.  This implies the following relationship: `tcp_retries2` >= `tcp_retries1`.  The two retry values configure how 'patient' your system should be when it comes to waiting on [RTOs].
...
Additionally, ARP can be configured with persistent static entries.  This typically done via a file.  Batches of static entries can be included in such a file.  The format line-by-line for such a file should be `<mac-address> <ip-address>`.  To load the file's entries into the system's ARP cache one can run `arp -f <file>`.  Typically the file that holds these entries should be stored under **/etc/ethers**.  
...
This file is one source of truth for DNS and is one way to manually define translations for certain hostnames.
...
* `nameserver`
The `nameserver` keyword is fairly self explanatory and follows the format of `nameserver <ip-address>` where `<ip-address>` is the IP address of the intended name server.  One can have up to `MAXNS`, defined in `<resolv.h>`,  `nameserver` entries in this file.  The resolver will query nameservers in the same order as they are written in the file.
...
* `ndots`
This option is in the format `ndots:n` configures the threshold,`n`, at which  an initial absolute query is made.  Since the default value for this option is 1, any name with at least 1 dot will first be queried as an absolute name before appending domains from `search`.  When less than `ndots` are present, the queries automatically begin appending elements in `<search-list>`.
* `timeout`
This opton is in the format `timeout:n` and configures the amount of time `n` that a resolver will wait for a response from a name server before retrying the query via another name server.
* `attempts`
...
### mtr
`mtr` is a command that combines the functionality of `traceroute` with that of `ping`. Take a look a look at [this article][mtr-primer] for a good primer for using `mtr` and interpreting its output.
...
3. Where does `dhclient` store old leases?
4. Please write a few stanzas that configures an interface called `test` and configures it so that it is brought up on boot and given the following address: `192.168.13.37/16`.  Where should this snippet be placed?
...
6. If we preferred name resolution be done dynamically rather than using static entries in **/etc/hosts** what file do we need to edit and what is the line we should add?
...
9. For each script. You will be doing a 2 step process.  **Only move onto another script, once you have finished resolving your current one.**
   ...
   **For each step you must explain the tools you used and how you came to your  conclusions i.e.** 
...
2. Start `ninja_port.py` again but now use `tcpdump` to monitor the conversation before connecting or communicating with the ninja. Additionally, please include the `-X` option when running `tcpdump`.  Please make an inference about which transport protocol (L4) was used to communicate with the ninja.  Why do you know it was this protocol?
3. The ninja isn't finished just yet, he's spent a few years training in a dojo and has mastered fireball (`火球`) jutsu.  He can use his new skills to tamper with your network stack, incinerating your attempts to catch him.  Run `sudo python3 advanced_ninja_port.py`.  Fix the damage he has done and then successfully send him a `found you` message again!
